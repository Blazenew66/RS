...
class Ranker:
    ...
    def rank_rs_scores(self, rs_raw_dict: Dict[str, float]) -> pd.DataFrame:
        """
        将 RS 原始值转换为排名分数并排序
        ...
        Args:
            rs_raw_dict: Dict[ticker, rs_raw]
        ...
        if not rs_raw_dict:
            ...
            return pd.DataFrame(columns=['ticker', 'rs_raw', 'rs_score', 'rank'])
        
        # 转换为 DataFrame
        df = pd.DataFrame([
            {'ticker': ticker, 'rs_raw': rs_raw}
            for ticker, rs_raw in rs_raw_dict.items()
        ])
        ...
        # 重新排列列顺序
        df = df[['ticker', 'rs_raw', 'rs_score', 'rank']]
